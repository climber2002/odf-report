...
module Parser
  ...
  class Default
    ...
    def parse_formatting(text)
      text.strip!
      text.gsub!(/<strong>\s*<em>(.+?)<\/em>\s*<\/strong>/) { "<text:span text:style-name=\"boldunderline\">#{$1}<\/text:span>" }
      text.gsub!(/<em>\s*<strong>(.+?)<\/strong>\s*<\/em>/) { "<text:span text:style-name=\"boldunderline\">#{$1}<\/text:span>" }
